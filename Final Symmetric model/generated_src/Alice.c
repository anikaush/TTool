...
void __userImplemented__Alice__sendInSeveralChunks(int m1__data) {
    printf("send in several chunks has been called\n");
     ...
     int m3__data = 1;
      ...
      printf("param0 check chunk 0 : %d \n",__params0); 
      
     size_t lenArr = sizeof(myLovelyArray) / sizeof(myLovelyArray[0]);
      
     for ( int j = 0 ; j <lenArr ; j ++){
    
       int m4__data = myLovelyArray[j];
        __params0[0] = &m4__data;
      makeNewRequest(&__req0, 2849, SEND_SYNC_REQUEST, 0, 0, 0, 1, __params0);
      __req0.syncChannel = &__System_chin__System_chout;
      __returnRequest = executeOneRequest(&__list, &__req0);
...
k__data) {
    ...
    unsigned char text[]="hello EURECOM";
...
void *mainFunc__Alice(void *arg){
  ...
  while(__currentState != STATE__STOP__STATE) {
    switch(__currentState) {
      ...
      makeNewRequest(&__req0, 6339, SEND_SYNC_REQUEST, 0, 0, 0, 1, __params0);
      __req0.syncChannel = &__System_chin__System_chout;
      __returnRequest = executeOneRequest(&__list, &__req0);
      clearListOfRequests(&__list);
      ...
      Alice__sendInSeveralChunks(m1__data);
